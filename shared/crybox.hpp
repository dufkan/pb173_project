...
public:
#endif
    key32 RK;     /*Root Key*/
    key32 CKs;    /*Chain Key for sending*/
    key32 CKr;    /*Chain Key for receiving*/
    ...
    key32 pubkey;
    std::map<std::pair<key32, uint16_t>, key32> SKIPPED;
     ...
     */
    static std::vector<uint8_t> create_header(key32 key, uint16_t PN, uint16_t N) {
        Encoder e;
        e.put(key);
     ...
     */
    std::tuple<key32, uint16_t, uint16_t> parse_header(std::vector<uint8_t>& msg) const {
        RefDecoder d{msg};
        key32 key;
        key = d.get_arr<32>();
        auto PN = d.get_u16();
     ...
     */
    void delete_skey(std::pair<key32,uint16_t> ipair) {
        key32 overkey;
